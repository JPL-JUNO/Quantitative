...
asserts = ['EURUSD', 'USDCHF', 'GBPUSD', 'USDCAD',
           'BTCUSD', 'ETHUSD', 'XAUUSD', 'SP500m', 'UK100']
...
def get_quotes(time_frame, year: int = 2017, month: int = 1, day: int = 1,
               asset='EURUSD'):
...
def signal(data):
    ...
    for i in range(len(data)):
        try:
            # long
            if data[i, 2] < data[i - 5, 2] and data[i, 2] < data[i - 13, 2] and \
                    data[i, 2] > data[i - 21, 2] and data[i, 3] > data[i - 1, 3] and data[i, 4] == 0:
                data[i + 1, 4] = 1
            elif data[i, 1] > data[i - 5, 1] and data[i, 1] > data[i - 13, 1] and \
...
def signal_pd(data: DataFrame) -> DataFrame:
    long_condition_1 = (data['low'] < data['low'].shift(5)) & (data['low'] < data['low'].shift(
        13)) & (data['low'] < data['low'].shift(21))
    long_condition_2 = data['close'] > data['close'].shift(3)

    data['long'] = (long_condition_1 & long_condition_2).astype(int).shift(1)

    short_condition_1 = (data['high'] > data['high'].shift(5)) & (
        data['high'] > data['high'].shift(12)) & (data['high'] < data['high'].shift(21))
    short_condition_2 = (data['close'] < data['close'].shift(3))

    data['short'] = (short_condition_1 &
                     short_condition_2).astype(int).shift(1)
    ...
    for i in range(len(sample)):
        ...
        if sample[i, 3] < sample[i, 0]:
            plt.vlines(x=i, ymin=sample[i, 3], ymax=sample[i, 0],
                       color='black', linewidth=1)
        if sample[i, 3] == sample[i, 0]:
            plt.vlines(x=i, ymin=sample[i, 3], ymax=sample[i, 0] + 1e-5,
                       color='black', linewidth=1)
    plt.grid()


def signal_chart(data, position: int, buy_column: int, sell_column: int,
                 window: int = 500):
    sample = data[-window:,]
        ...
        if sample[i, buy_column] == 1:
            ...
            ax.annotate(' ', xy=(x, y),
                        arrowprops=dict(width=9, headlength=11,
                                        headwidth=11, color='green'))
        elif sample[i, sell_column] == -1:

            ax.annotate(' ', xy=(x, y),
                        arrowprops=dict(width=9, headlength=-11, headwidth=-11,
                                        color='red'))


def performance(data, open_price, buy_column, sell_column,
                long_result_col, short_result_col, total_result_col):
    for i in range(len(data)):
        try:
            # 做多收益
