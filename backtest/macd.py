...
@CreatedTime  : 2024-05-26 16:45:41
@Email        : cuixuanstephen@gmail.com
@Description  : 
"""

import pandas as pd
from talib import MACD, EMA, BBANDS
import numpy as np
...
def divergences(row) -> int:
    ...
    mask2 = row["MACD_hist"] > row["macd_reach_low"]
    if mask1 and mask2:
        return 1
    else:
        return 0
...
def generate_data():
    raw = pd.read_csv("./data/603619.csv", parse_dates=["date"])
    ...
    raw = raw.assign(MACD_hist=macd, EMA=ema)
    raw = raw.assign(
    ...
    raw["macd_reach_low"] = raw["MACD_hist"].rolling(window=60).min()
    ...
    st.session_state["semi_auto_signal_index"] = semi_auto_signal_index
...
def reshow():
    ...
    if not data.empty:
        ...
        ax[1].plot(data.index + 1, data["MACD_hist"])
        ax[1].axhline(0, color="black")
        ax[1].fill_between(
            data.index + 1,
            0,
            data["MACD_hist"],
            where=data["MACD_hist"] > 0,
            color="red",
        )
        ax[1].fill_between(
            data.index + 1,
        ...
        ax[1].axhline(data.iloc[-1]["macd_reach_low"], color="red", linestyle="-.")
