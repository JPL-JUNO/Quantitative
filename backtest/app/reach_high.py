...
strategy_days = list(range(1, 30))
strategy_days.append("bh")
...
for underlying in tqdm(underlyings):
    ...
    for strategy_day in strategy_days[:-1]:
        raw["adj_signal"] = 0
    ...
    annualized_return = np.log(strategy_returns.max()) / (data_nums / 252)
    logger.info(
        f"{underlying!r} best hold day(s) {best_hold_days} return(s) {strategy_returns.max()}"
    )
    logger.info(f"{underlying!r} annualized returns {annualized_return:.2%}")
    raw[cols].plot()
    plt.tight_layout()
